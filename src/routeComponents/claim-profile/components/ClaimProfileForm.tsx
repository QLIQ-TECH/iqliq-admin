...
const brands: Brand[] = Array.from({ length: 8 }).map((_, idx) => ({
  id: idx,
  name: "Apple",
  handle: "@apple",
  logo: dummyBrand,
...
const ClaimProfileForm = () => {
  ...
  const onSubmit = (data: claimProfileValues) => {
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
                    {filteredBrands.map((brand) => {
                      ...
                      return (
                        <div
                          key={brand.id}
                          className={`flex flex-col justify-center items-center py-4 px-6 rounded-2xl border transition-300 cursor-pointer
