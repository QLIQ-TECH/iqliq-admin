...
import { kycSchema, type KycFormValues } from '@/validation/validation.schema';
import { getPresignedUrl } from '@/api/services/post.api';
import { useOnboarding } from '@/hooks/useOnboarding';
import type { UploadedKycData } from '@/lib/types';

const KycForm = () => {
  const [isUploading, setIsUploading] = useState<boolean>(false);
  const [filePreviews, setFilePreviews] = useState<{
    passportImage: string | null;
    emirateImage: string | null;
  ...
  const handleFileChange = (
    file: File | null,
    fieldName: 'passportImage' | 'emirateImage' | 'tradeImage' | 'vatImage',
  ...
  const onSubmit = async (values: KycFormValues): Promise<void> => {
    ...
    try {
      const uploadedData: UploadedKycData = {
  ...
  const FileUploadField = ({
    ...
    field: ControllerRenderProps<
      KycFormValues,
      'passportImage' | 'emirateImage' | 'tradeImage' | 'vatImage'
    >;
    fieldName: 'passportImage' | 'emirateImage' | 'tradeImage' | 'vatImage';
    label: string;
    id: string;
    ...
    const isPdf =
      (field.value instanceof File && field.value.type === 'application/pdf') ||
      (typeof preview === 'string' && preview.toLowerCase().includes('.pdf'));

    const isImage = preview && !isPdf;

    const getFileName = () => {
      if (field.value instanceof File) return field.value.name;
    ...
    return (
      <FormItem>
        ...
        <FormControl>
          <div>
            {preview ? (
              <div className="relative border-2 border-gray-300 rounded-xl p-3 bg-gray-50">
                <button
                  type="button"
                  onClick={() => handleRemoveFile(fieldName, field.onChange)}
                  className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1.5 hover:bg-red-600 transition shadow-md z-10"
                >
                ...
                <div className="flex items-center gap-3">
                  {isImage ? (
                    ...
                    <div className="flex-shrink-0 w-20 h-20 rounded-lg bg-white border border-gray-200 flex items-center justify-center">
              ...
              <label
                htmlFor={id}
                className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl cursor-pointer p-6 hover:border-blue-500 hover:bg-blue-50/50 transition"
              >
  ...
  return (
    <div className="text-black w-full space-y-5 sm:space-y-7 pr-7 max-h-[500px]">
        ...
        >
          ...
          <div className="flex flex-row gap-4 mt-7">
            <Button
              ...
              className="w-full bg-white rounded-3xl border py-5 border-blue-500 text-blue-500 hover:bg-blue-50 transition disabled:opacity-50"
            ...
            <Button
              type="submit"
              disabled={isUploading}
              className="w-full rounded-3xl py-5 bg-[#0082FF3D] text-[#0082FF] hover:bg-[#0082FF66] transition disabled:opacity-50"
            >
