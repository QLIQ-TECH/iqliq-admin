...
const CompetitorSelector = ({ competitors }) => {
    ...
    return (<div className="w-full text-black ">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <FormField control={form.control} name="competitors" render={({ field }) => (<FormItem>
                ...
                return (<label key={comp._id} className="cursor-pointer w-full">
                          <div className={`flex justify-between items-center border rounded-xl px-4 py-3 transition ${isSelected
                        ? 'border-blue-500 bg-blue-50'
                        : 'border-gray-300 bg-white hover:bg-gray-50'}`} onClick={() => {
                        if (isSelected) {
                            field.onChange(field.value.filter((c) => c !== comp._id));
                        }
                        else {
          ...
          <div className="flex flex-row gap-4 mt-7">
            <Button type="button" onClick={() => router.push('/onboarding/choose-category')} className="w-full bg-white sm:flex-1 rounded-3xl border py-5 border-blue-500 text-blue-500 hover:bg-blue-50 transition">
              Back
            </Button>
            <Button type="submit" className="w-full sm:flex-1 rounded-3xl border py-5 bg-[#0082FF3D] text-[#0082FF] hover:bg-[#0082FF3D] transition">
              Next
            </Button>
          </div>
        </form>
