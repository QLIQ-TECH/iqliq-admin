...
import { resetPasswordSchema } from "@/validation/validation.schema";
const ResetPasswordForm = () => {
    ...
    async function onSubmit(values) {
        ...
        if (!email || !token) {
            toast.error("Invalid link ❌", "Reset link is missing or invalid. Please request a new one.");
            return;
        }
        try {
            const response = await confirmForgotPasswordApi({
                email,
                token,
            ...
            if (response.success) {
                ...
                toast.error("Reset failed ❌", response.message || "Unable to reset password. Please try again.");
            }
        }
        catch (error) {
