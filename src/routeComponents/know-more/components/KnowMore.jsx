      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormField control={form.control} name="goals" render={({ field }) => (<FormItem>
                ...
                return (<label key={option} className="cursor-pointer w-full">
                        ...
                        if (isSelected) {
                            field.onChange(field.value.filter((s) => s !== option));
                        }
                        else {
          ...
          <div className="flex flex-row gap-4 mt-7">
            <Button type="button" className="w-full bg-white rounded-3xl border py-5 border-blue-500 text-blue-500 hover:bg-blue-50 transition" onClick={() => router.push('/onboarding/choose-language')}>
              Back
            </Button>
