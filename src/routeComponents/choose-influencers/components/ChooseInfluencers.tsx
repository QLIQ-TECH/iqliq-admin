...
const InfluencerSelector = ({ influencers: initialInfluencers }: Props) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [influencers, setInfluencers] =
    useState<Influencer[]>(initialInfluencers);
  ...
  return (
    <div className="w-full text-black">
      ...
      <div className="relative w-full mt-7">
        ...
        />
        {loading ? (
          <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-black animate-spin" />
        ) : (
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormItem>
                ...
                <FormControl>
                  <div 
                    ...
                    className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-80 overflow-y-auto"
                  >
                    {influencers?.map((inf) => {
                      const isSelected = field.value.includes(inf._id);
                      return (
                        <label key={inf._id} className="cursor-pointer w-full">
                          <div
          ...
          <div className="flex flex-row gap-4 mt-7">
            <Button
              type="button"
              className="w-1/2 bg-white rounded-3xl border py-5 border-blue-500 text-blue-500 hover:bg-blue-50 transition"
               onClick={() => router.push('/onboarding/choose-competitors')}
            >
              Back
            </Button>
            <Button
