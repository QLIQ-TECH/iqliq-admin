...
import type { SignUpRequest } from '@/types/brand';
...
const SignUpForm = ({ referralCode }: { referralCode?: string | null }) => {
  ...
  const [otpModal, setOtpModal] = useState<{
    ...
    open: boolean;
    value?: string;
  }>({ type: 'email', open: false, value: '' });

  const [otpSent, setOtpSent] = useState({ email: false, phone: false });
  const [verified, setVerified] = useState({ email: false, phone: false });

  const [emailTimers, setEmailTimers] = useState<TimerState>({
    resendCooldown: 0,
    otpExpiry: 0,
    isExpired: false,
  });

  const [phoneTimers, setPhoneTimers] = useState<TimerState>({
    resendCooldown: 0,
    otpExpiry: 0,
    isExpired: false,
  });
  ...
  const step1Form = useForm<Step1Values>({
    ...
    defaultValues: {
      ...
      gender: (step1Data.gender as "male" | "female") ?? undefined,
  ...
  const handleStep1Next = async (values: Step1Values) => {
    ...
    try {
      const res = await verifyRefferalCode(
      ...
      if (axiosError.response?.data?.message) {
        toast.error(
          `Referral code verification failed: ${axiosError.response.data.message}`
        );
   ...
   const handleFinalSubmit = async (step2Values: Step2FormValues) => {
      ...
      try {
        const mappedRole =
          ...
          response.data.user.role === 'super_admin'
            ? 'superadmin'
            : 'vendor';
        const u = response.data.user as unknown as {
          vendorId?: string;
  ...
  const handleOtpRequest = async (value: string, type: VerificationType) => {
    try {
      if (type === 'email') {
        const res = (await sendEmailOtp({ email: value })) as {
          success?: boolean;
          message?: string;
        };
        if (!res?.success) {
        ...
        const res = (await sendWhatsappOtp({ phone: value })) as {
          success?: boolean;
          message?: string;
        };
        if (!res?.success) {
  ...
  const handleResendOtp = async (type: VerificationType, value: string) => {
    try {
      if (type === 'email') {
  ...
  const handleChangeVerifiedValue = (type: VerificationType) => {
    ...
    setOtpModal((prev) =>
      prev.type === type ? { ...prev, open: false, value: '' } : prev
    );
  };
  ...
  const handleSendOtpAndOpenModal = async (
    ...
    if (ok) setOtpModal({ type, open: true, value: payload });
  };

  const handleOpenModal = (type: VerificationType, value: string) => {
  ...
  return (
    <div>
                  ...
                  <div className="relative pb-1.5">
                    <div className="relative">
                      ...
                      <VerificationButton
                        ...
                        verified={verified}
                        otpSent={otpSent}
                        onSendOtp={handleSendOtpAndOpenModal}
                        onOpenModal={handleOpenModal}
                        onChangeValue={handleChangeVerifiedValue}
                        toast={toast}
                      />
                    </div>
                    <FormMessage className="absolute left-1 -bottom-3 text-xs text-red-500" />
                ...
                <FormItem>
                  ...
                  <div className="relative pb-1.5">
                    <FormControl>
                      <Input
                        {...field}
                        value={referralCode ?? field.value ?? ''}
                        disabled={!!referralCode}
                        onChange={(e) => {
                          if (!referralCode) field.onChange(e.target.value);
                        }}
                        className="rounded-xl border-gray-300 py-5 px-3"
                      />
                    </FormControl>
                  ...
                  <div className="relative pb-1.5">
                    <FormControl>
                      ...
                      >
                        <ToggleGroupItem
                          value="male"
                          className="px-6 py-3 w-full rounded-xl border border-[#0082FF] data-[state=on]:bg-[#0082FF] data-[state=on]:text-white"
                        >
                          Male
                        </ToggleGroupItem>
                        <ToggleGroupItem
      ...
      {step === 2 && step1Data.email && (
        <Step2Form
          initialEmail={step1Data.email}
          onBack={() => setStep(1)}
      ...
      <OtpModal
        ...
        resendCooldown={currentTimers.resendCooldown}
        otpExpiry={currentTimers.otpExpiry}
        isExpired={currentTimers.isExpired}
        onClose={() => setOtpModal((prev) => ({ ...prev, open: false }))}
        onVerify={async (otp: number) => {
