...
const LanguageSelector = ({ languages }) => {
    ...
    return (<div className="w-full text-black space-y-4">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <FormField control={form.control} name="languages" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <div className="flex flex-col gap-3 max-h-80 overflow-y-auto">
                    {filteredLanguages.length === 0 && (<div className="text-center py-6 text-gray-500">
                        No languages found
                      </div>)}
                ...
                filteredLanguages.map((lang) => {
                    ...
                    return (<label key={lang._id} className={`w-full ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                            <div className={`flex justify-between items-center border rounded-xl px-4 py-3 transition ${isDisabled
                            ...
                            if (isDisabled)
                                return;
                            if (isSelected) {
                                field.onChange(field.value.filter((id) => id !== lang._id));
                            }
                            else {
                              ...
                              {isDisabled ? (<div className="w-5 h-5 border border-gray-300 rounded-full bg-gray-200"/>) : isSelected ? (<Check className="w-5 h-5 p-1 rounded-full bg-blue-500 stroke-white"/>) : (<div className="w-5 h-5 border border-gray-400 rounded-full bg-white"/>)}
                            </div>
                          </label>);
                })}
                  </div>
          ...
          <div className="flex flex-row gap-4 mt-7">
            <Button type="button" className="w-full rounded-3xl bg-white border py-5 border-[#0082FF] text-[#0082FF] hover:bg-blue-50 transition" onClick={() => router.push('/onboarding/virtual-assitance')}>
              Back
            </Button>
