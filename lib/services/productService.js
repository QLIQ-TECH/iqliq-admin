...
export const productService = {
  ...
  uploadProductImages: async (files) => {
    ...
    // Use media upload service for product images
    const mediaApiUrl = process.env.NEXT_PUBLIC_MEDIA_API_URL || 'http://localhost:5005';
    const response = await fetch(`${mediaApiUrl}/api/upload/product-images`, {
      method: 'POST',
      body: formData,
      headers: {
