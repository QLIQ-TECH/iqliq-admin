...
import { cartApi, metricsApi } from '../apiClient';

export const analyticsService = {
  // Vendor dashboard: scoped metrics from cart/order service (paid orders for this vendor only)
  getVendorAnalytics: async (vendorId, params = {}) => {
    if (!vendorId) {
      throw new Error('vendorId is required for vendor analytics');
    }
    const id = encodeURIComponent(String(vendorId).trim());
    return await cartApi.get(`/metrics/vendor/${id}`, params);
  },
