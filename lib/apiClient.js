  ...
  async handleResponse(response) {
    ...
    if (!response.ok) {
      if (response.status === 401 && typeof window !== 'undefined') {
        ...
        if (!onLoginPage) {
          console.warn('🔴 [ApiClient] 401 Unauthorized detected. Clearing session.');
          ...
          // We don't force a redirect here anymore; we let the AuthContext/Components 
          // handle the null user state for a smoother transition.
        }
      }
