...
// Registered by AuthContext so that a 401 after a failed refresh triggers a clean logout
// instead of a raw localStorage wipe that leaves React state out of sync.
...
const API_BASE_URLS = {
  ...
  WALLET: process.env.NEXT_PUBLIC_WALLET_API_URL || 'https://backendwallet.qliq.ae/api',
  POSTS: process.env.NEXT_PUBLIC_POSTS_API_URL || 'https://backendposts.qliq.ae/api',
};
...
class ApiClient {
  ...
  getAuthToken() {
    ...
    if (token) {
      console.log('🔍 [ApiClient] getAuthToken - found token:', {
        hasToken: true,
  ...
  async handleResponse(response) {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        console.warn('🔴 [ApiClient] 401 from', response.url, '— token may lack required claims for this service.');
      }
...
export const walletApi = new ApiClient(API_BASE_URLS.WALLET);
export const postsApi = new ApiClient(API_BASE_URLS.POSTS);
