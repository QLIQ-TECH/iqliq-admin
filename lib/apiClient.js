...
class ApiClient {
  ...
  getStoredAuthContext() {
    if (typeof window === 'undefined') {
      return { tokens: null, user: null, email: null };
    }

    try {
      const tokensData = localStorage.getItem('qliq-admin-tokens');
      const userData = localStorage.getItem('qliq-admin-user');
      const tokens = tokensData ? JSON.parse(tokensData) : null;
      const user = userData ? JSON.parse(userData) : null;
      const email = user?.email || null;
      return { tokens, user, email };
    } catch (error) {
      console.error('❌ [ApiClient] Error reading auth context from localStorage:', error);
      return { tokens: null, user: null, email: null };
    }
  }
  ...
  getAuthToken() {
    const { tokens } = this.getStoredAuthContext();
  ...
  async getFreshToken() {
    const refreshedAccessToken = await this.refreshWithStoredToken();
    if (refreshedAccessToken) {
      return refreshedAccessToken;
    }

    try {
      const refreshEndpoints = ['/refresh', '/refresh-token'];
      for (const endpoint of refreshEndpoints) {
        const response = await fetch(`${API_BASE_URLS.AUTH}${endpoint}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  async refreshTokenRequest(refreshToken, email) {
    const refreshEndpoints = ['/refresh', '/refresh-token'];
    let lastError = null;

    for (const endpoint of refreshEndpoints) {
      try {
        const payload = email ? { refreshToken, email } : { refreshToken };
        const response = await fetch(`${API_BASE_URLS.AUTH}${endpoint}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!nextAccessToken) {
          lastError = new Error('Refresh endpoint did not return access token');
          continue;
        }

        return { data, nextAccessToken };
      } catch (error) {
        lastError = error;
      }
    }
    ...
    throw new Error('No refresh endpoint available');
  }

  // Try refreshing access token using locally stored refresh token
  async refreshWithStoredToken() {
    if (typeof window === 'undefined') return null;

    try {
      const { tokens, email } = this.getStoredAuthContext();
      if (!tokens?.refreshToken) return null;
      const { data, nextAccessToken } = await this.refreshTokenRequest(tokens.refreshToken, email);
      ...
      localStorage.setItem('qliq-admin-tokens', JSON.stringify(nextTokens));
      return nextAccessToken;
    } catch (error) {
      console.error('Error refreshing token from local refresh token:', error);
      return null;
    }
  }
  ...
  async handleResponse(response, originalRequest = null) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        // Access token may be expired; try refresh token once before logging out
        if (originalRequest && !originalRequest._retriedWithRefresh) {
          const refreshedAccessToken = await this.refreshWithStoredToken();
          if (refreshedAccessToken) {
            const retryRequest = { ...originalRequest, _retriedWithRefresh: true };
            return await this.retryRequest(retryRequest, refreshedAccessToken);
          }
        }

        // Unauthorized after refresh attempt - clear auth data and redirect to login
        if (typeof window !== 'undefined') {
