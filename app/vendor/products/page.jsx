...
export default function VendorProductsPage() {
  ...
  const extractProductsAndPagination = (response, fallbackPage = 1) => {
  ...
  const fetchProducts = async (pageToLoad = 1, showTableLoading = true) => {
    try {
      ...
      const { list, pag } = extractProductsAndPagination(response, pageToLoad);
      setProducts(list);
  ...
  const fetchStats = async () => {
    try {
      ...
      const { list: allProducts } = extractProductsAndPagination(statsResponse, 1);
