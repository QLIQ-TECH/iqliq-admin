...
export default function CreatorsPaymentHistoryPage() {
  ...
  const fetchPage = useCallback(async (pageToLoad = 1) => {
    try {
      ...
      const response = await gigsService.getCreatorsPaymentHistory({
        page: pageToLoad,
        limit: LIMIT
      });
      ...
      console.error('Creators payment history fetch failed:', e);
      setRows([]);
      setPagination((prev) => ({
        ...prev,
        total: 0,
  ...
  return (
    <CreatorsPaymentShell
      title="History"
      description="Completed payments to creators (accepted gigs with paid status)."
    >
